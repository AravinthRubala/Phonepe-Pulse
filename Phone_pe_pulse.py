# Note : Before running Phone_pe_pulse progrm. Please check once related folder "PhonePe_data" available or not. 
# If it is available please delete the folder. Because we are extracting data from "Git clone"
# same like that delete csv file (agg_trans,agg_user_state_brand,agg_user_state_reg,map_trans,map_user,top_trans_district,top_trans_pincode,top_user_district,top_user_pincode)

import git
# PhonePe pulse GitHub repository data Cloning
...
agg_user_state_reg()
agg_user_state_reg().shape
agg_user_state_reg().isna().sum().sum()
agg_user_state_reg().describe
agg_user_state_reg().info
...
agg_user_state_brand()
agg_user_state_brand().shape
agg_user_state_brand().info
agg_user_state_brand().describe
agg_user_state_brand().isna().sum().sum()
...
top_trans_district().info
top_trans_district().describe
top_trans_district().isna().sum().sum()
...
top_trans_pincode().info
top_trans_pincode().describe
top_trans_pincode().isna().sum().sum()
...
top_user_district()
top_user_district().shape
top_user_district().info
top_user_district().describe
top_user_district().isna().sum().sum()
...
top_user_pincode()
top_user_pincode().shape
top_user_pincode().info
top_user_pincode().describe
top_user_pincode().isna().sum().sum()

# Coverting Data frames to csv files
agg_trans().to_csv('agg_trans.csv',index=False)
agg_user_state_reg().to_csv('agg_user_state_reg.csv',index=False)
agg_user_state_brand().to_csv('agg_user_state_brand.csv',index=False)
map_trans().to_csv('map_trans.csv',index=False)
map_user().to_csv('map_user.csv',index=False)
top_trans_district().to_csv('top_trans_district.csv',index=False)
top_trans_pincode().to_csv('top_trans_pincode.csv',index=False)
